...
const LandDescriptionCardMain = ({ land }: ILandDescriptionCardProps) => {
  return (
      ...
      <div className="w-4/6 flex-col ml-3 mt-4 mr-4">
        <div className="flex">
          ...
          <div className="flex">
            ...
            <p className="text-gray-500 mr-3 ml-1">{land?.landUse}</p>
          </div>
        </div>
        ...
        <div className="flex text-traverse-yellow my-3 font-semibold">
          ...
          <p className="ml-3">{currencyFormat(land?.price ?? 0)}</p>
        </div>
      </div>
    </div>
  );
...
export default LandDescriptionCardMain;
